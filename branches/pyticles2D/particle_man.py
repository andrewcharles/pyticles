#!/usr/bin/env python
# insert pyglet license and disclaimer here.
""" Particle-Man rips off pyglet's astraea example code (Alex Holkner)
    and uses modules from the pyticles smooth particle program for
    a more fun and interactive experience.
...
class Player(WrappingSprite, key.KeyStateHandler):
    ...
    def update(self, dt):
        ...
        rotation_x = math.cos(radians(-self.rotation))
        rotation_y = math.sin(radians(-self.rotation))
...
class ParticleSystem(WrappingSprite):

    def __init__(self,img,batch=None):
        self.p = particles.Particles(3)
        super(ParticleSystem, self).__init__(img,WINX/2,WINY/2,batch=batch)
        p = self.p
        self.nl = particles.neighbour_list.NeighbourList(p,2.0)
        self.p.nlists.append(self.nl)
        self.p.nl_default = self.nl
        self.nl.add_force(particles.forces.HookesForce(p,p,p.nlists[0])) 
        self.xmap = WINX / float(particles.XMAX)
        self.ymap = WINY / float(particles.YMAX)
    ...
    def draw(self):
        ...
        #    self.img.blit(p.r[i,0],p.r[i,1])
            ...
            for angle in range(6):
                a = radians(angle*60)
                glVertex2f(r[0]+math.sin(a)*radius,r[1]+math.cos(a)*radius)
            glEnd() 
...
class MainMenu(Menu):
    def __init__(self):
        ...
        self.items.append(MenuItem('Pyticles',200,run_pyticles))
        self.items.append(MenuItem('Instructions', 160, 
                                   begin_instructions_menu))
        self.items.append(MenuItem('Options', 120, begin_options_menu))
        self.items.append(MenuItem('Quit', 100, sys.exit))
        self.reset()
...
particle_system = ParticleSystem(resource.image('asteroid1.png'),wrapping_batch)
