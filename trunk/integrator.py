...
def euler(get_state,calc_derivs,get_derivs,set_state,dt):
    """ Simple euler integration. So simple it's pointless to wrap it like
        this, except as an exercise to work out the interface.

        state: vector describing the system's state.
        get_state: a function to obtain the system state
	calc_derivs: calculates the derivatives for the system
	get_derivs: gathers the derivates from the system into a vector
        scatter_state: a function to scatter the state vector to a more
                       descriptive objects (e.g. an n-body system organised in particles)
        dt: timestep size
